...
- [ ] **Interface coverage gaps (deferred)** — translate via `t()` the dynamically-rendered chrome (atlas map title/sub, zoom label Macro/Meso/Micro, person/theme placeholders, pk-error/ref-label, drawer button labels); Timeline event titles/gerações still treated as content
