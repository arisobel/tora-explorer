...
- [ ] **editor.html: campos de tradução EN/ES/HE por fato** *(imediato, sem backend)* — no editor local, mostrar o PT canônico + campos para as traduções e gravar em `text_i18n`/`summary_i18n`/`caption_i18n` direto no JSON. Tradução de conteúdo assistida, fato a fato. Depende só da decisão do schema i18n de conteúdo acima — independe da migração SQLite. Bom candidato a próxima feature do editor.
