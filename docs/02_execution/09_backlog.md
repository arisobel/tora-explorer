...
- [~] Design pass: typography scale, spacing rhythm, and color hierarchy tokens reviewed across all 6 tabs — **foundation done (2026-06-14)**: `--text-xs…3xl`, `--lh-*`, `--space-1…8` tokens added to `:root`; applied to page frame (`.page` padding, `.sec-title` 28px tighter leading, `.sec-sub` with full-width hairline divider unifying all tabs). Next: propagate `--space-*`/`--text-*` to components (cards, pillars, grids, sub-headings)
