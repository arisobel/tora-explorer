...
- [x] **Pessukim language selector shipped** — Hebrew fixed + 1 selectable translation (EN/PT/ES); `PK_TRANS` map + `pkSetTransLang()` (persists in localStorage); `pkLoad()` fetches HE + chosen translation in parallel with automatic English fallback when the language is absent for that book/chapter (shows a discreet note in the ref-label); `pkState.enTexts`→`trTexts`; header column relabels to the shown language. Verified live: PT/ES return 22-verse arrays for Esther 1; Daniel-PT correctly falls back to EN. Foundation slice of the i18n objective (2026-06-14)
- [x] **Global site language switcher (i18n interface) shipped** — 🌐 PT/EN/ES/עב in `<nav>`; `setLang()` persists `site-lang`, sets `dir`/`body.rtl`, applies catalog. Engine: selector-based, keyed by exact PT source text, translates **text nodes** so nested badges/icons survive; `t(key, ptDefault)` for JS-generated strings. Catalogs `data/i18n/{en,es,he}.json` (81 keys each, full parity, all keys verified present in HTML). Covers nav tabs, all section titles/subtitles, Estrutura drill-path + flow boxes + pillar headers + book-group/oral/gen labels + academias + braitot, Atlas toolbar/panels/filters, Chumash book buttons + atlas head, Timeline legend + oral-bar title, and the full Pessukim control set. Fixed a latent bug: `drawerGoToPessukim` matched the tab by `textContent` (would break under translation) → now `[data-page]`. Deferred to content phase: chip tooltips, timeline event descriptions, rabbi-name Hebrew forms, dynamic atlas labels. JS syntax + catalog parity + text-node replacement logic all validated (2026-06-14)
- [x] **Content i18n (translatable facts) — schema + authoring + consumption** — bloco `i18n` aninhado por objeto: `fact.i18n.{en,es,he}.{text,topic,caption}` e `summary.i18n.{lang}.short`, PT canônico como fallback (`text_he` segue sendo o passuk, não tradução). **App:** helper `trContent(obj, field)` aplicado no drawer (texto/tópico/legenda/resumo), atlas2d, drill cards e timeline drilldowns; `illustrationHTML` aceita legenda traduzida; `setLang` re-renderiza drawer/timeline/drill já abertos (`_drawer.current`). **Editor:** seção *3 · Traduções* — alvo (unidade `summary.short` ou fato), *Carregar alvo* mostra PT-fonte + textareas EN/ES/HE (HE RTL) pré-preenchidos, *Aplicar* grava no objeto certo (vazio remove, limpa `i18n` órfão). Documentado em `data/SCHEMA.md`. Sintaxe de ambos os scripts validada; resolver testado contra o JSON real de Ester (2026-06-15)
- [x] **Ester traduzida (EN/ES/HE) — primeiro livro-piloto de conteúdo** — `data/nach/megillot/05-esther-purim-deliverance.json` recebeu blocos `i18n` em todos os 22 facts (`text`+`topic`), nas 7 legendas (`caption`) e no `summary.short`, nos três idiomas. PT permanece canônico; HE narrativo (português→hebraico, nomes próprios em forma hebraica: אֲחַשְׁוֵרוֹשׁ/מָרְדֳּכַי/אֶסְתֵּר/הָמָן) preenche o modo RTL. Aplicado via script único (fora do repo) e validado: JSON parseável, paridade en/es/he completa em todos os campos, 0 lacunas (2026-06-15)
- [x] **Bereshit traduzida (EN/ES/HE) — piloto do Chumash** — `data/parashiot/genesis/01-bereshit.json`: `i18n` nos 20 facts (`text`+`topic`), nas 5 legendas de ícone e no `summary.short`, nos três idiomas; nomes em forma hebraica (אָדָם/חַוָּה/קַיִן/הֶבֶל/נֹחַ). Junto, estendido o consumo de `caption` traduzida também para marcadores `icon` (não só `image`): `visualMarkerHTML(v, capOverride)` recebe a legenda traduzida no drawer, e o `editor.html` (trResolve) passa a oferecer o campo `caption` para qualquer visual com legenda. Validado: paridade completa, 0 lacunas, sintaxe de index/editor OK (2026-06-16)
- [x] **Noach traduzida (EN/ES/HE)** — `data/parashiot/genesis/02-noach.json`: `i18n` nos 21 facts (`text`+`topic`), nas 8 legendas de ícone e no `summary.short`, nos três idiomas; nomes em forma hebraica (נֹחַ/שֵׁם/חָם/יֶפֶת/כְּנַעַן/תֶּרַח), termos-chave (תֵּבָה/מַבּוּל/קֶשֶׁת/מִגְדַּל בָּבֶל). Paridade completa, 0 lacunas (2026-06-16)
- [x] **Lech Lecha + Vayera + Chayei Sarah traduzidas (EN/ES/HE)** — `03-lech-lecha.json` (17 facts, 4 legendas), `04-vayera.json` (15 facts, 3 legendas), `05-chayei-sarah.json` (12 facts, 3 legendas) — `text`+`topic`+`caption`+`summary.short` nos três idiomas. Aplicadas num único script (fora do repo). Paridade completa nas três, 0 lacunas. Gênesis: 5/12 parashiot traduzidas (2026-06-16)
- [x] **Drawer: passagem Sefaria segue o idioma global da plataforma** — `drawerLoadPassage` deixou de fixar Hebraico+Inglês e agora escolhe a versão de tradução da Sefaria conforme `currentLang` via `SITE_LANG_TO_SEFARIA` (pt→`portuguese`, en→`english`, es→`spanish`, he→sem coluna de tradução, só o passuk). Mesmo fallback para Inglês da aba Pessukim quando a tradução escolhida não existe no livro (com nota "tradução indisponível neste livro, exibindo Inglês" no rodapé), e cache de passagem re-chaveado por `ref::lang` para não servir idioma obsoleto. Validado com screenshots headless carregando Genesis 1:1-2 em PT ("No princípio, Deus criou os céus e a terra."), ES ("En el principio del crear de Dios...") e HE (só hebraico, drawer espelhado à esquerda). Sem erros JS (2026-07-04)
- [x] **Design foundation site-wide — esquema frio da proposta aplicado a todo o app** — o esquema de CSS de `docs/05_references/design_proposal_01/estrutura_tora_atlas_visual.html` foi promovido do Atlas 2D para o site inteiro via camada de tokens. **`:root` remapeado**: neutros frios (`--bg #f7f9fc`, `--surface2 #f4f6fa`, `--border #dbe3f1`), acento primário azul (`--gold*` agora carregam `#0f67e8`/`#0a52c0` por compatibilidade; ouro real movido para `--hal`/`--hal2 #db8b00`), adicionados `--ink`/`--navy`/`--purple*`, raios maiores (`--r 12px`, `--r2 18px`) e sombras frias suaves (`--shadow-1/2` em `rgba(10,32,70,…)`). **Tipografia híbrida** (decisão do usuário): Inter em toda a interface (`body` + shell do Atlas), Crimson Pro preservado em títulos de seção, termos hebraicos e logo. **replace_all** das famílias rgba quentes → frias/azul (gold-accent→azul, sombras/bordas quentes→frias, chips azul/verde antigos→azul/verde da proposta). **Semântica de domínio corrigida** para casar com a proposta: Torá Escrita = azul, Torá Oral = verde, Halachá = ouro (novo `.chip-gold` + `.badge-gold` ouro; cards Mikra/Talmud/Halachá, Guemará→verde). Validado com screenshots headless em Estrutura, Timeline, Mishna, Chumash, Pessukim, drawer, Atlas 2D e **modo hebraico/RTL** — sem regressões, sem erros JS, 0 valores quentes remanescentes. Fundação para o objetivo estratégico "design/appearance improvement pass" (2026-07-04)
- [x] **Quick wins do HTML da proposta Melachim** — fonte `docs/05_references/design_proposal_01/estrutura_tora_atlas_visual.html` analisado e três detalhes absorvidos: níveis de zoom nomeados sob a régua (Macro/Geral/Contexto/Detalhe/Profundidade, ativo destacado a cada render), labels de texto alternando acima/abaixo dos losangos de Acontecimentos, e card "Sobre o período" no painel lateral (nome+hebraico, `timeline.description`, período AM, abrangência ref inicial–final — tudo do `index.json`). Não absorvidos de propósito: rail lateral do mock (duplicaria a régua canônica) e eixo a.E.C. (AM é canônico). Validado com screenshot headless (2026-07-04)
- [x] **Timeline contextual expandida — fatia 1 (Chumash)** — novo painel multi-faixa no dock do Atlas quando um livro do Chumash está selecionado (zoom ≥ 2), inspirado no mock Melachim: eixo AM com ticks adaptativos (`atlasTlxTickStep`), faixa Parashiot (barras posicionadas por `anno_mundi_start/end`, clique abre o drawer), faixa Acontecimentos (losangos de milestones filtrados por `importance` — 1ª aplicação real da regra de promoção visual: zoom 2 → 4-5, zoom 3+ → 3-5), faixa Personagens (spans derivados de `mart.byCharacter`, top 8 em até 3 linhas empacotadas). Chips de era somem enquanto a expandida está ativa; régua AM de precisão preservada. Zero fontes de dados novas. Nach fica para quando existir estado de livro selecionado no Atlas (`nach-book` hoje abre drawer direto); fallback por `key_events` já embutido. Validado com screenshot headless (Gênesis: Bereshit/Noach largas, patriarcais à direita, Noach/Avraham/Sara/Yaakov/Yossef na faixa de personagens) (2026-07-04)
- [x] **Atlas 2D responsivo + dedup de zoom (BUG-04 corrigido)** — dock da timeline saiu do `position: fixed` para o fluxo do documento (mesmo visual quando cabe na tela, zero sobreposição quando não cabe); rail lateral esquerdo removido e controles de zoom deduplicados (régua de pontos no topo = única affordance + `Reset` na toolbar); breakpoints novos: topbar reflui ≤1280px, painel lateral desce abaixo do mapa e macro cards empilham ≤1100px, lista de eras com scroll horizontal; grid do stage simplificado para coluna única. Validado com screenshots headless (Chrome/Edge) em 1366x768, 1920x1080, 768x1024 e ~460px + probe JS de overflow horizontal (DOCW == VW). Nenhuma mudança de comportamento JS (2026-07-03)
